...
function OutputJournal({ job }) {
	...
	const formatTimeForJournal = (time) => {
		...
		const [hoursValue, minutesValue = "0", secondsValue = "0"] = time.split(":")
	...
	const updateScrap = (index, field, value) => {
		setScrapList((currentList) =>
			currentList.map((item, itemIndex) => {
				...
				if (field === "code") {
					const selectedScrap = scrapCodes.find(
						(scrap) => (scrap.Code || scrap.code || scrap.No) === value,
					)

					if (selectedScrap) {
						updatedItem.description =
							selectedScrap.Description || selectedScrap.description || ""
					}
				}
	...
	useEffect(() => {
		const fetchScrapCodes = async () => {
			try {
				const res = await getReq("api/psl/scrap", user.token)

				// Backend already returns an array
				setScrapCodes(res || [])
			} catch (error) {
				console.error("Failed to fetch scrap codes:", error)
			}
		}
						...
						<div key={`scrap-${index}`} className='output-journal-row-card'>
							<label className='output-journal-field'>
								...
								<select
									...
									{scrapCodes.map((scrap, scrapIndex) => (
										<option
											key={scrapIndex}
											value={scrap.Code || scrap.code || scrap.No}>
											{scrap.Code || scrap.code || scrap.No}
											{" - "}
											{scrap.Description || scrap.description}
										</option>
									))}
								</select>
							</label>
